...
//Update User//
        ...
        $telp_cust = $_POST["telp_cust"];
        if($password==""){
            $queryEdit = mysqli_query($koneksi, "UPDATE user SET nama='$nama', hak_akses='$hak_akses' WHERE id_user='$id_user'") or die($koneksi);
            if ($queryEdit){
                echo "
                    <script>
            ...
            $queryEdit = mysqli_query($koneksi, "UPDATE user SET nama='$nama', password=MD5('$password'), hak_akses='$hak_akses' WHERE id_user='$id_user'") or die($koneksi);
            if ($queryEdit){
                echo "
                    <script>
                        alert('Berhasil Update User!');
